...
export const Radio: React.FC<RadioProps> = ({ options, label, value, size = 'base', onChange, ...rest }) => {
  ...
  return (
    <fieldset>
      {label && <legend className="text-xs font-medium text-gray-700 mb-1">{label}</legend>}

      <div className="space-y-1">
        {options.map((option) => (
          <div
            key={option.value}
            className={classNames('relative', option?.description ? 'items-start' : 'items-center')}
          >
            <div className={classNames('grid grid-cols-[18px_1fr] items-center gap-x-2')}>
              <input
