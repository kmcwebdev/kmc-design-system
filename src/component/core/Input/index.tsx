  ...
  return (
    <div className={classNames('flex flex-col', className && className)}>
      ...
      <div className="relative flex w-full">
        {leading && (
          <span className="text-sm font-medium z-10 pointer-events-none absolute inset-y-0 left-0 pl-3 flex items-center">
            {leading}
          </span>
        )}
      ...
      {error && (
        <span className={classNames('text-xs font-normal', error ? 'text-main-danger' : 'text-gray-blue-5')}>
          {error}
        </span>
      )}
    </div>
